...
function ExcelExportScreen() {
  ...
  async function doExport() {
    setExporting(true);
    await loadSheetJS();
    const header = ['部品名', '部品番号', ...LOCS, '合計'];
    const aoa = [header, ...list.map((p) => [p.name, p.no, ...LOCS.map((l) => p.locs[l] || 0), LOCS.reduce((s, l) => s + (p.locs[l] || 0), 0)])];
    const ws = window.XLSX.utils.aoa_to_sheet(aoa);
    const wb = window.XLSX.utils.book_new();
    window.XLSX.utils.book_append_sheet(wb, ws, '在庫');
    window.XLSX.writeFile(wb, `部品在庫_${today()}.xlsx`);
    setExporting(false);
    setToast('📊 Excelをダウンロードしました');
  ...
  return (
    <div style={{
      ...
      <Header connected={connected} title="Excelエクスポート" right={<div style={{ display: 'flex', gap: 6 }}><Button variant="ghost" size="sm" onClick={() => history.back()}>‹ 戻る</Button><Button variant="ghost" size="sm" onClick={() => { window.location.href = '在庫管理 ホーム画面.html'; }}>🏠</Button></div>} />
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Card padding={14}>
          ...
          <div style={{ fontSize: 'var(--text-md)', fontFamily: 'var(--font-mono)' }}>部品在庫_{today()}.xlsx</div>
        </Card>
      </div>
      ...
      <div style={{
        ...
        <Button variant="outline" disabled={!connected || list.length === 0 || exporting} style={{ width: '100%', height: 54, fontSize: 'var(--text-md)' }} onClick={doExport}>
          {exporting ? '生成中…' : '📊 Excelをダウンロードする'}
        </Button>
      </div>
...
window.ExcelExportScreen = ExcelExportScreen;
